  ...
  try {
    ...
    if (req.cookies.jwt!=undefined) {
      ...
      try {
        ...
        if (req.user) {
          console.log("existe user router/home", req.user.nombre, req.user.imagen, req.user.dni);
          res.render("lista18",{ user: req.user.nombre, logo: req.user.imagen, userid: req.user.dni});
...
router.get('/api/propiedades/:id',async (req,res)=>{
  console.log("entro en api/propiedades",  [req.params] )
  let filtro = [req.params.id]
  if (filtro > 0){
    filtro = " where s.dni = " +  [req.params.id]
  }
  else{
    filtro =""
  }
...
router.get('/vercard/:id',async (req,res)=>{
  ...
  try {
    ...
    console.log("usuarios", inmueble.length);
    ...
    try {
     ...
     try {
      ...
      res.render('vercard', { inmueble, califica, largo_cali: califica.length, promedio: resultado_promedio[0].promedio });
     } catch (error) {
      {
        console.log(error);
      }
...
router.get('/vermapa/:id',async (req,res)=>{
  ...
  try {
    ...
    if (req.params.id>0){
      ...
      const [inmueble] = await pool.query("SELECT * from inmobiliaria where dni= ? ", [req.params.id] )  
      console.log("inmueble", inmueble[0])  
     
      console.log("inmueble.nombre", inmueble[0].nombre, inmueble[0].imagen, req.params.id )
      res.render('vermapa', { user: inmueble[0].nombre, logo: inmueble[0].imagen, userid: req.params.id});
    }
    else{
      console.log("entro en else")
      res.render('vermapa', { user: false, logo: false, userid: null});
    }
    } catch (error) {
      {
        console.log(error);
...
router.get('/api/provincias',async (req,res)=>{
  ...
  try {
    const [filas] = await pool.query("SELECT DISTINCT p.descripcion as prov, p.id_provincia as id_provincia, p.descripcion as descripcion, p.id_provincia as cod FROM inmueble x INNER JOIN provincia p on x.id_provincia = p.id_provincia;");
...
router.get('/api/localidades',async (req,res)=>{
  ...
  try {
    const [filas] = await pool.query("SELECT DISTINCT p.descripcion as loc, p.id_localidad as id_localidad, p.descripcion as descripcion, p.id_localidad as cod FROM inmueble x INNER JOIN localidad p on x.id_localidad = p.id_localidad;");
    console.log("localidad", filas.length);
    res.send(filas);
  } catch (error) {
    {
...
router.get("/login", (req, res) => {
  ...
  if (req.user) {
    res.render("login", { alert: false, user: req.user[0], userid:  req.user[0] });
  } else {
...
router.get("/lista18", async (req, res) => {
  console.log("ejecuto lista18 router parael usuario", req.userid);
  console.log("req.user", req.user);
  //const [filas] = await pool.query("SELECT u.dni as dni, x.nombre,  x.apellido, x.calificacion, x.celular, x.codigopostal, x.imagen, l.descripcion as localidad, p.descripcion as provincia, GROUP_CONCAT(DISTINCT o.descripcion SEPARATOR ' - ') as oficio,  x.presentacion from usuario_oficio u INNER JOIN oficio o on u.id_oficio = o.id_oficio INNER JOIN usuario x on x.dni = u.dni INNER JOIN localidad l on l.id_localidad = x.id_localidad INNER JOIN provincia p on p.id_provincia = x.id_provincia GROUP by u.dni order by x.nombre, x.apellido ;");
  //console.log("usuarios lista 18", filas.length);
  if (req.user) {
    res.render("lista18", { alert: false, user: req.user[0], userid: req.userid });
  } else {
...
router.get("/prop_nueva", async (req, res) => {

    console.log("router /prop_nueva",req);
   // res.render("lista10",{ user: null}); // este anda
   try {
    const paises = await pool.query("SELECT * FROM pais order by descripcion");
    console.log("paises", paises[0]);
    try {
        ...
        try {
          ...
          if (req.cookies.jwt!=undefined) {
            ...
            try {
              req.user = await estalogueado(req.cookies.jwt);
              console.log("req.usr ", req.user.nombre, req.user.dni);
              if (req.user) {
                console.log("existe user router/home", req.user.nombre, req.user.imagen);
                res.render("prop_nueva",{ user: req.user.nombre, logo: req.user.imagen,
                  pais_data: paises[0],
                  categoria_data: cat[0],
                  alert: false,
                  inmobili: req.user.dni,
                  userid: req.user.dni
                });
                //res.render("lista10",{ user: req.user});
                //res.render("home", { registro: results, user: req.user });
            ...
            console.log("no tiene cookies") 
            //res.render("lista10",{ user: false});
            res.render("prop_nueva",{ user: false});
            //res.render("home", { registro: results, user: false });
          }
...
async function estalogueado(caracteres) {
  ...
  try {
    ...
    try {
      const [pp] = await pool.query("SELECT * FROM inmobiliaria WHERE dni = ?", [
        decodificada.id,
      ]);
...
*/
  ...
  console.log("router /new_user await usuario_nuevo",pp)
  console.log("pp", pp.nombre, pp.dni, pp.id_inmobiliaria, pp.logo_inmobiliaria )

let alertmesagge = "Felicidades " + pp.nombre
  res.render("login",{   alert: true,
    alertTitle: "¡Propiedad Grabada !",
    ...
    ruta: "home",
    userid: pp.dni,
    user: pp.nombre, dni: pp.dni, apellido: pp.apellido, logo:pp.logo_inmobiliaria})
