...
export const  usuario_nuevo = async (req, tienearchivo, filename) => {
  ...
  const id_inmobiliaria = req.body.id_inmobiliaria;
  const logo_inmobiliaria = req.body.logo_inmobiliaria;
 ...
 // let passHash = await bcryptjs.hash(pass, 8);
  ...
  //console.log(celular);
  let registro = await pool.query("INSERT INTO inmueble SET ?", {
  ...
  console.log("carga ok", dni, registro, nombre,id_inmobiliaria,logo_inmobiliaria);
 return {nombre,dni,id_inmobiliaria,logo_inmobiliaria }
};
...
pie = ``
  ...
  const mailOptions = {
    ...
    cc: "",
    to: correo,
    subject: "Mercado Inmobiliario - Blanqueo de contraseña",
    text: "",
    html: contentHTML,
  };
